...
const MaxAmt = 1.0; // max amount to steal per hack (decimal%) //lower helps maintain stability
export const MaxProcess = 10000; //Maximum allowed concurrent processes
...
const RamUse = 0.95 // Amount of your total ram to allow profiles to build from
...
//	if (asd && !Array.isArray(asd.hmTargets)) { asd.hmTargets = []; }
	...
	do {
		...
		for (const tar of tars) {
			...
			let ret = { tar: tar, value: 0, score: 0 };
			let hN = 1;
			do {
				await ns.sleep(0);
				...
				if ((ret.score > 0) && ((ret.cost - RamUse * asd.totRam) <= 0) && ((ret.amt - MaxAmt) <= 0) && ((ret.value - bests[bi].value) > 0)) {
					bests[bi] = ret;
					if (bests.length > 0) {
